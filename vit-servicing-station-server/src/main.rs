...
async fn main() {
    ...
    let paths: Vec<PathBuf> = fs::read_dir(&settings.block0_path).unwrap().filter_map(|path| {
    ...
    if paths.is_empty() {
        std::process::exit(ApplicationExitCode::EmptyBlock0Folder.into());
    }

    let context =
        v0::context::new_shared_context(db_pool, paths, &settings.service_version);
