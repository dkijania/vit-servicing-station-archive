...
fn convert_fund(fund: &Fund) -> Vec<String> {
    ...
    vec![
        ...
        stage_dates.finalize_proposals_start.to_string(),
        stage_dates.proposal_assessment_start.to_string(),
        stage_dates.assessment_qa_start.to_string(),
        stage_dates.snapshot_start.to_string(),
        stage_dates.voting_start.to_string(),
        stage_dates.voting_end.to_string(),
        stage_dates.tallying_end.to_string(),
        results_url.as_ref().unwrap_or(&"".to_string()).to_string(),
        survey_url.as_ref().unwrap_or(&"".to_string()).to_string(),
    ]
}
...
fn convert_advisor_review(review: &AdvisorReview) -> Vec<String> {
    vec![
        ...
        review
            ...
            .unwrap_or(&"".to_owned())
            .to_string(),
        review.feasibility_rating_given.unwrap_or(0).to_string(),
        review
            .feasibility_note
            .as_ref()
            .unwrap_or(&"".to_owned())
            .to_string(),
        review.auditability_rating_given.unwrap_or(0).to_string(),
        review
            .auditability_note
            .as_ref()
            .unwrap_or(&"".to_owned())
            .to_string(),
        (review.ranking.unwrap_or(ReviewRanking::Good) as u8 == 0).to_string(),
        (review.ranking.unwrap_or(ReviewRanking::Excellent) as u8 == 1).to_string(),
    ]
}
