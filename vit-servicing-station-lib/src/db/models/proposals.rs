        ...
        let challenge_info = match challenge_type {
            ChallengeType::Simple | ChallengeType::Native => {
                ProposalChallengeInfo::Simple(simple::ChallengeInfo {
                    proposal_solution: row.27.clone().unwrap_or_else(|| "".to_string()),
                })
            }
            ChallengeType::CommunityChoice => {
                ProposalChallengeInfo::CommunityChoice(community_choice::ChallengeInfo {
                    proposal_brief: row.28.clone().unwrap_or_else(|| "".to_string()),
                    proposal_importance: row.29.clone().unwrap_or_else(|| "".to_string()),
                    proposal_goal: row.30.clone().unwrap_or_else(|| "".to_string()),
                    proposal_metrics: row.31.clone().unwrap_or_else(|| "".to_string()),
                })
            }
        };
        FullProposalInfo {
